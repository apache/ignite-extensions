        ...
        for (int attempts = 3; attempts > 0; attempts--) {
            try {
                ...
                DatadirCleanupManager purgeMgr =
                        new DatadirCleanupManager(dataDir.getAbsolutePath(), logDir.getAbsolutePath(), 24, 5);
                purgeMgr.start();
                ...
                try {
                    quorumPeer = new QuorumPeer(configuration.getServers(), dataDir, logDir, configuration.getElectionAlg(), position, 2000, 5, 2, cnxnFactory);
                } catch (IOException e) {
