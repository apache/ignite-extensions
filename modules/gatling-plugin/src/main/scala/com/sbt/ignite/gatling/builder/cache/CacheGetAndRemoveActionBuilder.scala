...
import com.sbt.ignite.gatling.action.cache.CacheGetAndRemoveAction
...
class CacheGetAndRemoveActionBuilder[K, V](
    ...
    override def build(ctx: ScenarioContext, next: Action): Action =
        new CacheGetAndRemoveAction(requestName, cacheName, key, withKeepBinary, withAsync, checks, next, ctx)
}
