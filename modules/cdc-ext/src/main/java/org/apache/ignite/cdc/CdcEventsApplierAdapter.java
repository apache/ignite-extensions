 ...
 */
public abstract class CdcEventsApplierAdapter<K, V> {
    /** Maximum batch size. */
     ...
     * @param log Logger.
     */
    public CdcEventsApplierAdapter(int maxBatchSize, IgniteLogger log) {
    ...
    protected int apply(Iterable<CdcEvent> evts) throws IgniteCheckedException {
        ...
        for (CdcEvent evt : evts) {
            ...
            if (evt.value() != null) {
                ...
                updBatch.put(key, toValue(evt.value(), ver));
            }
            else {
    ...
    /** @return Value. */
    protected abstract V toValue(Object val, GridCacheVersion ver);

    /** Stores DR data. */
    protected abstract void putAllConflict(int cacheId, Map<K, V> drMap) throws IgniteCheckedException;

    /** Removes DR data. */
    protected abstract void removeAllConflict(int cacheId, Map<K, GridCacheVersion> drMap) throws IgniteCheckedException;
}
